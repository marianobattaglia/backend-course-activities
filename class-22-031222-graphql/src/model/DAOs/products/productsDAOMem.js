module.exports = class ProductsDAOMem {
  ...
  getById(id) {
    ...
    if (!elem) throw new Error(`Error getById at DAO: element not found`);
    return elem;
  }
  ...
  update(elem) {
    ...
    if (index == -1) throw new Error(`Error update at DAO: element not found`);
  ...
  deleteById(id) {
    ...
    if (index == -1)
      throw new Error(`Error deleteById at DAO: element not found`);
