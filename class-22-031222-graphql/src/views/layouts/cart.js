const productosApi = {
  get: () => {
    return fetch("/productos").then((data) => data.json());
  },
};
...
const carritosApi = {
  crearCarrito: () => {
    ...
    return fetch("/cart", options).then((data) => data.json());
  },
  getIds: () => {
  ...
  getProds: (idCarrito) => {
    return fetch(`cart/${idCarrito}/productos`).then((data) => data.json());
  },
...
document.getElementById("btnAgregarAlCarrito").addEventListener("click", () => {
  ...
  if (idCarrito && idProd) {
    ...
    alert("Debes seleccionar un carrito y un producto");
  }
});
